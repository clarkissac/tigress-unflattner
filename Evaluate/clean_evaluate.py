...
def main():
    dir = "F:\BAKK2\Evaluate\Samples_clean"
    ...
    for root, dirs, files in os.walk(dir):
        for filename in files:
            ...
            os.popen(f'"{ida_dir}\\ida64.exe" -A -S"F:\\FH\\BAKK2\\Samples\\Evaluate\\notflatten_evaluate.py {",".join(functions)} {filename}" "{os.path.join(root, filename)}"').read()
